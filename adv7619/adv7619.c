...
static int adv7619_device_init(void)
{
	...
	ret = adv7619_i2c_write(pclient_io, 0x03, 0x20); // 8-bit 4:2:2 DDR mode 2 (ITU-656 mode)
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_io, 0x05, 0x2E); // AV Codes Off
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_io, 0x0B, 0x44); // Power up part
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_io, 0x14, 0x7F); // Max Drive Strength
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_io, 0x33, 0x40); // LLC DLL enable
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_io, 0x20, 0xF8); // Manually assert hot plug on port A
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_ksv,  0x79, 0x1); //  HDCP 
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x9B, 0x03); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC1, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC2, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC3, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC4, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC5, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC6, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC7, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC8, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xC9, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xCA, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xCB, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0xCC, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
...
#if 0
	ret = adv7619_i2c_write(pclient_hdmi, 0x00, 0x00); // Set HDMI Input Port A
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x83, 0xFE); // Enable clock terminator for port A
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
...
------------------------
#else
	val = adv7619_i2c_read(pclient_hdmi, 0x6c);
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_read is failed, errornum == %d\n", ret);
		return ret;
	}
	//dbg_print("q1111 val== %d\n", val);
	msleep(5);

	val |= 1;

	//dbg_print("q2222 val== %d\n", val);

	ret = adv7619_i2c_write(pclient_hdmi, 0x6c, val); 
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	val = 0;
	val = adv7619_i2c_read(pclient_io, 0x20);
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_read is failed, errornum == %d\n", ret);
		return ret;
	...
	val &= ~(1<<6);
	//dbg_print("q5555 val== %d\n", val);

	ret = adv7619_i2c_write(pclient_io, 0x20, val); 
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	msleep(5);
		...
		ret = adv7619_i2c_write(pclient_hdmi, 0x00, 0x01); // Set HDMI Input Port B
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x83, 0xFF); // Enable clock terminator for port B
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x6F, 0x0C); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x85, 0x1F); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x87, 0x70); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x8D, 0x04); // LFG
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x8E, 0x1E); // HFG
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x1A, 0x8A); // unmute audio
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x57, 0xDA); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x58, 0x01); // ADI recommended setting
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x03, 0x98); // DIS_I2C_ZERO_COMPR
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
	...
	ret = adv7619_i2c_write(pclient_hdmi, 0x75, 0x10); // DDC drive strength
	if(ret < 0)
	{
		dbg_print("adv7619_i2c_write is failed, errornum == %d\n", ret);
		return ret;
